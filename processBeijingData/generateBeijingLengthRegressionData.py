...
def generateLengthHeuristicsTrainSet():
...
#    print(np.array(final_batch).shape)
      for j in range(1, len(final_batch[0]) - 1):
...
#        y_batch = np.zeros(wait_actions.shape) + 10000
...
#          y_batch[k, 0] = len(final_batch[0]) - 1 - j
        y_batch = np.zeros(len(final_batch)) + len(final_batch[0]) - 1 - j
#        for k in range(1):
        heuristics_batches.append([np.array(final_batch)[:, 0:j].tolist(), np.array(final_batch)[:, j].tolist(), np.array(final_batch)[:, -1].tolist(), y_batch.tolist()])
#    tracker.print_diff()
    if (counter + 1) % 100 == 0:
      print(len(heuristics_batches))
      print("batches generated:{}...".format(counter))
    if (counter + 1) % 500 == 0:
      pickle.dump(heuristics_batches, open("/data/wuning/AstarBeijing/beijing_Q_learning_policy_surpervised_length_heuristicsTestSet"+str(counter), "wb"))
      heuristics_batches = []
...
#generateTestDataSet()
  generateLengthHeuristicsTrainSet()
